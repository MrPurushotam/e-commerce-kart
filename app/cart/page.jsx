...
const decreaseQuantity = (productId) => {
  ...
  return (
    <div className='w-full p-2 '>
      ...
      <div className='border-2 border-black rounded-md w-full mx-auto grid grid-cols-[2fr_1fr] h-[80vh] overflow-hidden'>
        <div className='h-full w-full p-2 space-y-2 overflow-auto'>
          {cart.length<1 && <span className='text-center mx-auto block font-semibold'>Your cart is empty</span>}
          {cart?.map((item) => {
            
            return (
              <SimpleProductComponent key={item.product.id} product={item.product} quantity={item.quantity} currency={currency} decreaseQuantity={decreaseQuantity} increaseQunatity={increaseQuantity} deleteItem={removeProduct} />
            )
          })}
        </div>
        <div className='w-full h-full bg-yellow-400 p-2 rounded-md space-y-3'>
          ...
          <div className='overflow-auto h-[55%]'>
            ...
            {cart?.map((item) => {
              ...
              return (
                <SimpleCostComponenet key={item.product.id} price={item.product.price} quantity={item.quantity} title={item.product.title} total={currentTotalPrice} currency={currency} />
              )
            })}
          </div>
          ...
          <div>
            ...
            <div className='font-bold flex justify-between items-center'>
              ...
              <InputBox
                ...
                value={discountCode}
                className={"w-2/3 p-1 text-semibold uppercase bg-gray-200 text-black rounded-md border-2 border-gray-500"}
                inputTitle={"Apply Discount Code to get sexy discounts!"}
                placeholder={"Coupon"} />
            </div>
            <Button className={`w-full py-2 border-2 border-black rounded-md bg-gray-100 hover:bg-gray-200 font-bold text-lg my-2`} content={"Checkout"} />
          </div>
        </div>
      </div>
    </div>
